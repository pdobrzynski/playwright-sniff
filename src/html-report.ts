...
export function generateReportHTML(data: TestReport): string {
  ...
  console.log(calculateBadnessPercentage(avgLoadTimeTotal, data.reportData[0].slowThreshold))
  ...
  const performanceIndicator = overallPassed ? 
    ...
    `<div class="performance-indicator bad">
       <span class="indicator-icon">✗</span>
       <span class="indicator-text">Test Failures Detected</span>
     </div>`;
  ...
  <body>
    ...
    <footer class="container">
      <p>Generated by Playwright Sniff - ${new Date().getFullYear()}</p>
    </footer>
